...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name',type=str,default='ffhq-256-config-e-003810',
    ...
    if args.code_type=='w':
        ...
        Gs=LoadModel(dataset_name=dataset_name)
        all_images=GetImg(Gs,num_img=1000,num_once=num_once,output_path=output_path,resize=args.resize)
        tmp=output_path+'/images'
        np.save(tmp,all_images)

    elif args.code_type=='images_10':
        Gs=LoadModel(dataset_name=dataset_name)
        all_images=GetImg(Gs,num_img=25,num_once=num_once,output_path=output_path,resize=args.resize)
        tmp=output_path+'/images'
        np.save(tmp,all_images)
